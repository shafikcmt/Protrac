...
export function FinishingQCScanForm({
  ...
  // twice still re-applies. Mirrors the sewing-QC scan form's prefill.
  useEffect(() => {
    if (!prefill) return;
  ...
  const performSubmit = (data: FormData) => {
    // Defects only apply to fail/rework.
    onSubmit({
      ...data,
        ...
        <CardContent className="px-4">
          <Form {...form}>
            <form
              ...
              {/* Inline defect quick-select — only for Fail / Rework. Same
                  component the Sewing QC scanner uses. */}
              {needsDefects && (
                <DefectsQuickSelect
                  defects={defects}
