...
export function OrderCombobox({
  ...
  const filteredLocal: Order[] = React.useMemo(() => {
    ...
    return orders.filter((o) =>
      [o.order_number, o.style_name, o.size_name, o.color_name]
        .filter(Boolean)
  ...
  React.useEffect(() => {
    let cancelled = false;
    const canSearchRemote =
      searchQuery.trim().length >= 2 && filteredLocal.length === 0;
    if (!canSearchRemote) {
      setRemoteItems([]);
    ...
    const handle = setTimeout(async () => {
      try {
        const queries: Record<string, any> = { search: searchQuery.trim() };
