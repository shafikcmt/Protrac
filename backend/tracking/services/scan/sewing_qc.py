...
def _validate_user_scanner(user: "User") -> "Scanner":
    """Validate user has assigned sewing QC scanner."""
...
def _validate_garment_for_sewing_qc(garment: Garment, scanner: "Scanner") -> None:
    """Validate garment can be processed for sewing QC."""
    if garment.sewing_line != scanner.production_line:
        raise ValueError("Garment was not produced on this sewing line")
...
def _determine_garment_status_from_qc(qc_status: str) -> str:
    ...
    if qc_status == QualityCheckStatus.PASS:
        return GarmentStatus.SEWING_QC_PASS
    elif qc_status == QualityCheckStatus.FAIL:
        return GarmentStatus.SEWING_QC_FAIL
    elif qc_status == QualityCheckStatus.REWORK:
        return GarmentStatus.SEWING_QC_REWORK
    else:
...
def process_sewing_qc_scan(
    ...
    """Process sewing QC scan for garment."""
    ...
    # Validate garment for sewing QC
    _validate_garment_for_sewing_qc(garment, scanner)

    # Handle different scenarios based on current status. Re-evaluation is now
    # detected automatically from the garment's QC history — no manual flag: a
    # garment previously failed or sent to rework can simply be re-scanned, and
    # when it passes it counts as output like any other pass.
    if garment.status == GarmentStatus.ISSUED_FOR_ASSEMBLY:
        # First QC check.
    ...
    elif garment.status == GarmentStatus.SEWING_QC_PASS:
        # Already passed and counted as output. Block a re-scan so output/DHU
        # totals are never double-counted.
        raise ValueError(
            f"Garment already passed sewing QC (status: {garment.status}). "
            "It has already been counted as output."
        )

    else:
        raise ValueError(f"Garment not ready for sewing QC (status: {garment.status})")
    ...
    # Set assembly completion time if QC passed
    if new_status == GarmentStatus.SEWING_QC_PASS:
        garment.assembly_completed_at = now()
...
def get_sewing_qc_info(
    ...
    """Get sewing QC history for user's scanner."""
    # Validate user and scanner
    scanner = _validate_user_scanner(user)
